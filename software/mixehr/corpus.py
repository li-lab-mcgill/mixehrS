...
from mixehr.utils import generate_data
...
# parser filter by ID
parser_filterByID = subparsers.add_parser('filterid', help="Select specific patients")
parser_filterByID.add_argument("-id", "--patient_id_file", help="File containing the ID of patients")
...
class Corpus(Dataset):
    ...
    def build_corpus_sample(K, T, W, D, M, a=1, b=1, tau=4):
        ...
        '''
        y, b, x, z, g = generate_data(K, T, W, D, M, a, b, tau)
    ...
    def build_from_mixehr_fileformat(data_path: str, meta_path: str, label_path: str, persist_path: str = None,
        ...
        def __read_response__(patient_ids, labels):
            ...
            for i, patient in enumerate(pbar):
                response = labels[labels['SUBJECT_ID'] == patient]['HOSPITAL_EXPIRE_FLAG'].item()
                response = -1 if np.isnan(response) else response
        ...
        def __read_patients__(data, y, patient_ids, type_ids, vocab_ids):
            ...
            with tqdm(total=number_records) as pbar:
                ...
                for i, row in enumerate(data.iterrows()):
                    ...
                    type_id = type_ids[row[1]]
                    word_id = vocab_ids[row[2]]
                    freq = row[3]
        ...
        if ignore_missing_labels:
            ids = labels.loc[labels['HOSPITAL_EXPIRE_FLAG'].isin([0, 1])]['SUBJECT_ID'].unique()
        else:
            ids = data[0].unique()
            ids = labels.loc[labels['SUBJECT_ID'].isin(ids)]['SUBJECT_ID'].unique()
        if top_n:
            ids = ids[:top_n]

        data = data[data[0].isin(ids)]
        ids = data[0].unique()

        for i, patient_id in enumerate(ids):
            patient_ids[patient_id] = i

        # data = data[patient_ids.keys().isin(data[0])]
        for i, type_id in enumerate(data[1].unique()):
            type_ids[type_id] = i + 1
        for i, word_type in enumerate(data[2].unique()):
            vocab_ids[word_type] = i + 1
    ...
    @staticmethod
    def filterById(corpus, toStore, file_id):
        ...
        patient_ids = pd.read_csv(file_id, header=None)[0].values
        index = -1
        patients = []
        C = 0
...
def run(args):
    ...
    if cmd == 'process':
        path = os.path.join(BASE_FOLDER, 'mimic_data.txt')
        meta = os.path.join(BASE_FOLDER, 'mimic_meta.txt')
        labels = os.path.join(BASE_FOLDER, 'death_label.txt')
        ...
        c = Corpus.read_corpus_from_directory(BASE_FOLDER)
        Corpus.filterById(c, STORE_FOLDER, args.patient_id_file)
...
if __name__ == '__main__':
    ...
    # run(parser.parse_args(['process', '-im', '-n','150','/Users/cuent/Downloads/processed_new','/Users/cuent/Downloads/processed_new/delete2']))
    # run(parser.parse_args(['split', '/Users/cuent/Downloads/processed_new/delete2', '/Users/cuent/Downloads/processed_new/delete2']))
    ...
    # corpus_dir = '/Users/cuent/Downloads/processed_new/delete1'
    # corpus = Corpus.read_corpus_from_directory(corpus_dir)
    # for c in corpus:
    #     print(c)
